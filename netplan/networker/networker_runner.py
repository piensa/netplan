...
from infrastructure_planning import nx_shp as nn_xx
...
class NetworkerRunner(object):
    ...
    def run(self):
        ...
        nn_xx.write_shp(msf, self.output_directory)
        # write geojson too
...
def filter_min_node_subnetworks(g, min_node_count):
    ...
    subgraphs = [g.subgraph(component).copy() for component in components]
    ...
    filtered_non_grid = union_all_wrap(list(filter(
        lambda sub: len(sub._node) >= min_node_count, non_grid_connected)))

    grid = union_all_wrap(list(grid_connected))
...
def merge_network_and_nodes(network, demand_nodes,
    ...
    if single_network:
        ...
        keys_list = list(network.coords.keys())
        parent = keys_list[0]
        subgraphs.add_component(parent, budget=network._node[parent]['budget'])
        for node in keys_list[1:]:
            subgraphs.add_component(node, budget=network._node[node]['budget'])
...
def load_existing_networks(filename, budget_value=0, prefix=None):
    ...
    nx.set_node_attributes(geo_net, {n: budget_value for n in geo_net.nodes()}, 'budget')
    ...
    nx.set_edge_attributes(geo_net, {(u, v):
                           distance(map(geo_net.coords.get, [u, v]))
                           for u, v in geo_net.edges()}, 'weight')
...
def load_node_metrics(filename, x_column="X", y_column="Y",
    ...
    for node in geo_nodes.nodes():
        if budget_column in geo_nodes._node[node]:
            geo_nodes._node[node] = {'budget':
                                    geo_nodes._node[node][budget_column]}
        else:
            geo_nodes._node[node] = {'budget': budget_value}
