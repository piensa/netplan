...
def spherical_distance(coord_pair, radius=MEAN_EARTH_RADIUS_M):
    ...
    """
    n_by_2_by_2_array = np.array([[(x,y) for (x,y) in coord_pair]])
    return spherical_distance_haversine(n_by_2_by_2_array, radius)[0]
...
def make_bounding_box_array(coords):
    ...
    """
    coord_array = np.array([(x, y) for (x, y) in coords])
