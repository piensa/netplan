...
import infrastructure_planning.nx_shp as nnxx
import warnings
import os
import re
...
def read_shp_networkx_graph(shp, simplify=True, geom_attrs=True):
    ...
    geom_attrs: bool
        ...
        NOTE:  if these attributes are available, write_shp iill use them
...
def write_shp(geograph, shp_dir):
    ...
    nnxx.write_shp(nx_coord_graph, shp_dir)
...
def to_geojson(geograph):
    ...
    def node_to_geojson(node_id):
        ...
        node_geo_dict["properties"].update(geograph._node[node_id])
    ...
    def edge_to_geojson(node_from, node_to):
        ...
        edge_dict = geograph.edges[(node_from, node_to)]
