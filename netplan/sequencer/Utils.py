...
def prep_data(network, metrics, loc_tol=.5):
    ...
    if(len(network_nodes) > 0 and
       isinstance(network_nodes[0], collections.abc.Iterable)):
    ...
    node_df = DataFrame(network._node).T
    keys = [x for x in network._node.keys()]
    # cast coords to tuples (hashable)
    node_df["coords"] = [tuple(network._node[x]["coords"]) for x in keys]
    # join (x,y) coords to create a 'metrics coords'
    metrics['m_coords'] = list(map(tuple, metrics[['X', 'Y']].values))
    ...
    metrics = pd.merge(metrics, node_df, on='m_coords').sort_index()
    ...
    closest_match = metrics.loc[metrics.groupby('m_coords').apply(find_closest).values]
...
def parse_cols(df):
    columns = df.columns
    strip_chars = lambda x: '.' if x not in list(string.ascii_letters) + list(map(str, range(10))) else x 
